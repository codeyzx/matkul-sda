...
// 			else /* Belum berada di akhir List */
// 			{
// 				// newNode = Alokasi(Info(First(L)));
// 				// if (i == 0)
// 				// 	Next(newNode) = Nil;
// 				printf("%d ", Info(First(L)));
// 				// First(L) = Next(First(L));
// 				// newNode = Alokasi(Info(Next(First(L))));
...
/*	diprint. Jika list kosong, hanya menuliskan "List Kosong" */
{
	...
	else /* List memiliki elemen */
	{
		...
		for (;;)
		{
			...
			else /* Belum berada di akhir List */
			{
				...
				{
					// newNode = Alokasi(Info(First(L)));
					// Next(newNode) = Alokasi(Info(First(L)));
					// newNode = Next(newNode);
					temp = Alokasi(Info(First(L)));
					Next(temp) = newNode;
					newNode = temp;
				}
				else
	...
	for (;;)
	{
		...
		{
			printf("%d ", Info(reversedNode));
			reversedNode = Next(reversedNode);
		}
	}
}
